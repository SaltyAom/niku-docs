...
const BenefitSection: BenefitSectionComponent = ({ title, details, code }) => (
    ...
    >
        <section className={tw`flex flex-col justify-center pr-8 flex-1`}>
            <h2 className={tw`m-0 text-5xl sm:text-6xl text-gray-900 dark:text-gray-100 font-normal mb-4`}>
                {title}
            </h2>
            {details.map((content, index) => (
                // eslint-disable-next-line react/no-array-index-key
                <p className={tw`text-gray-700 dark:text-gray-400 text-2xl mb-1`} key={index}>
                    {content}
                </p>
            ))}
        </section>
